= Unblu Visitor Single Sign-On (SSO)
:source-highlighter: rouge

Visitor SSO Implementation Tutorial: https://unblu.github.io/unblu-visitor-sso-sample/[HTML]

== Run Sample Application

.Build from source
[source,bash]
----
./gradlew bootRun
----

.Run using Docker
[source,bash]
----
docker run --rm -p 8080:8080 ghcr.io/unblu/unblu-visitor-sso-sample
----

Then open http://localhost:8080.

.cURL samples
[source,bash]
----
# List generated JWKs
curl -s localhost:8080/api/jwk | jq
# Issue a new JWT
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"username": "pmuster", "email": "peter.muster@example.com", "firstname": "Peter", "lastname": "Muster"}' \
  localhost:8080/api/token | jq
----

== Configuration

The configuration `unblu.serverUrl` needs to point to a running Unblu installation with enabled ID propagation based on JWT.

.Change `src/main/resources/application.properties` to reconfigure this Sample App
[source,ini]
----
include::src/main/resources/application.properties[tag=config]
----

== Test Data

.Key pair for testing
[source]
----
-----BEGIN PUBLIC KEY-----

dwIDAQAB
-----END PUBLIC KEY-----

-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
----

== Known Issues

None