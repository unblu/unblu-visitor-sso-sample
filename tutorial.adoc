...
. The visitor's browser sends the JWT to Unblu (`POST /unblu/rest/v3/authenticator/loginWithSecureToken`)
...
The claims must be named as configured in the http://docs.unblu.com/en/docs/latest/reference/configuration-properties.html#com-unblu-platform-server-core-auth-configuration-claimmappingconfiguration[`claimMapping`^] in the Unblu configuration.
...
The session can go on be much longer than the JWT expiration.
...
.Call `logout` from the visitor's browser
[source,javascript,indent=0]
----
